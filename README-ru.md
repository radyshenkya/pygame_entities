...
Так же здесь применяется класс `Vector2` из модуля `pygame_entities.utils.math`. По сути этот представляет вектор с двумя числами - `x` и `y`, и поддерживает матиматические операции. Позиция каждой сущности хранится именно в виде объекта класса `Vector2`.
